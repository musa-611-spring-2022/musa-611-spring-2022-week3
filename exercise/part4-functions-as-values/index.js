...
let reduce = function (arr, func, initial) {
  let reducenum = initial;
  for (let i = 0; i <= arr.length - 1; i++) {
    let item = arr[i];
    reducenum = func(reducenum, item);
  }
  return reducenum;
};
...
console.log('reduce success #3:', _(
  reduce([1, 2, 3, 4, 5, 4, 4], (x, y) => [y, ...x], []),
).isEqual([4, 4, 5, 4, 3, 2, 1]));
...
let sumSquares = (arr) => reduce(arr.map((x) => x * x), add, 0);
