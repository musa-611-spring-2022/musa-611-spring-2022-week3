...
let countItem = (arr, item) => {return arr.filter(x => x == item).length};
...
let mapIncrement = (arr) => {return arr.map(plusOne)};
...
let mapTriple = (arr) => {return arr.map(timesThree)};
...
let reduceSum = (arr) => {return arr.reduce(add)};
