...
let countItem = (arr, item) => arr.filter((n) => (n === item)).length;
...
let isOdd = (n) => n % 2 !== 0;
let filterOdd = (arr) => arr.filter((number) => isOdd(number));
...
let plusOne = (n) => n + 1;
let mapIncrement = (arr) => arr.map((number) => plusOne(number));
...
let mapTriple = (arr) => arr.map((number) => timesThree(number));
...
let add = (n1, n2) => n1 + n2;
let reduceSum = (arr) => arr.reduce((newNumber, currentNumber) => add(newNumber, currentNumber));
...
let reduceProduct = (arr) => arr.reduce((x, y) => multiply(x, y));
